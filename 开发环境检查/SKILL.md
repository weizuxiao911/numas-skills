---
name: 开发环境检查
description: 检查开源贡献所需开发环境 (GitHub CLI 安装与授权), 自动执行只读检查, 缺失时引导用户完成安装与登录
---

# 开发环境检查

确保用户本机具备开源贡献所需的基础工具链, 重点是 **GitHub CLI (`gh`)** 可用:
1. `gh` 已安装
2. `gh` 已登录 GitHub 账号, 且 token 有效 (具备 repo/fork 权限)
